...
int readerCount = 0;
char fileName[50];  // Для хранения имени файла

void WriteEmployeesToFile() {
    FILE* file;
    fopen_s(&file, fileName, "wb");  // Открываем файл для записи с введенным именем
    if (file != NULL) {
        fwrite(emps, sizeof(Employee), empsNum, file);  // Записываем всех сотрудников
        fclose(file);
...
void ReadEmployeesFromFile() {
    FILE* file;
    fopen_s(&file, fileName, "rb");  // Открываем файл для чтения с введенным именем
    if (file != NULL) {
        Employee tempEmp;
...
int main() {
    ...
    scanf_s("%s", fileName, 50);  // Ввод имени файла с консоли
    ...
    // Читаем сотрудников из бинарного файла и выводим их
